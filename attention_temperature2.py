...
def process_segment(args, start, end, llm, sampling_params, dataset):
    total_examples = len(dataset)
    assert 0 <= start < end <= total_examples, f"Invalid segment [{start}, {end})"

    selected_indices = list(range(start, end))
    subset = dataset.select(selected_indices)
    print(f"Processing examples [{start}, {end}) → {len(subset)} samples")
    ...
    print(f"\n🎉 Segment [{start}, {end}) completed and saved.\n")


def main(args):
    dataset = load_dataset("HuggingFaceH4/MATH-500", split="test")
    total_examples = len(dataset)  # 应为 500

    # 验证整体范围（可选）
    if args.end > total_examples:
        args.end = total_examples
    ...
    while current < args.end:
        seg_start = current
        seg_end = min(current + segment_size, args.end)

        # 检查是否已存在该段结果（支持断点续跑）
        model_name_clean = args.model_name.split("/")[-1]
        segment_dir = f"{seg_start}-{seg_end}"
        output_dir = os.path.join(
            args.output_base_dir,
            model_name_clean,
            "MATH-500",
        ...
        # 如果该段所有 run 文件都存在，则跳过
        all_exist = True
        for run_id in range(1, args.num_samples + 1):
            if not os.path.exists(os.path.join(output_dir, f"run{run_id}.json")):
                all_exist = False
                break

        if all_exist:
            print(f"⏭️  Segment [{seg_start}, {seg_end}) already exists. Skipping.")
        else:
            print(f"🚀 Processing segment [{seg_start}, {seg_end})")
            process_segment(args, seg_start, seg_end, llm, sampling_params, dataset)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--start", type=int, default=0, help="Overall start index (default: 0)")
    parser.add_argument("--end", type=int, default=500, help="Overall end index (default: 500)")
    ...
    parser.add_argument("--segment_size", type=int, default=50, help="Not used directly; hardcoded to 50")
